...
const SAMPLE_URLS = {
  ...
  serviceLocationPages: [
    ...
    '/social-media-management-galway-city',
    '/content-marketing-oranmore',
    '/email-marketing-salthill',
    '/ppc-advertising-galway-city',
    '/branding-strategy-oranmore',
    '/video-marketing-salthill',
    '/web-hosting-maintenance-galway-city',
  ],
};
...
test.describe('Kraft Agency Website Validation', () => {
  ...
  test('Navigation links work correctly', async ({ page }) => {
    ...
    const servicesLink = page.locator('a[href*="service"], a:has-text("Services"), a:has-text("services")').first();
  ...
  test('Individual service pages load without 404s', async ({ page }) => {
    const errors: string[] = [];
    const consoleErrors: string[] = [];
    ...
    for (const url of SAMPLE_URLS.servicePages) {
      try {
        ...
        console.log(`Service page ${url} - Status: ${response?.status()}, Title: ${title}`);
  ...
  test('Individual location pages load without 404s', async ({ page }) => {
    ...
    for (const url of SAMPLE_URLS.locationPages) {
      try {
        ...
        console.log(`Location page ${url} - Status: ${response?.status()}, Title: ${title}`);
  ...
  test('Service+Location combination pages load without 404s', async ({ page }) => {
    ...
    for (const url of SAMPLE_URLS.serviceLocationPages) {
      try {
        ...
        console.log(`Service+Location page ${url} - Status: ${response?.status()}`);
  ...
  test('Click-to-call buttons exist on pages', async ({ page }) => {
    ...
    console.log(`Found ${phoneCount} click-to-call button(s)`);
